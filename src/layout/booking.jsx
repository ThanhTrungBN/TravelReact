...
const Booking = function () {
    ...
    useEffect(() => {
        ...
        const decodedOrderInfo = decodeURIComponent(orderInfoValue);
        console.log(decodedOrderInfo)
        setOrderId(decodedOrderInfo.split("-")[0]);
    }, []);
    useEffect(() => {
        const fetchBooking = async () => {
            try {
                const bookingg = await bookingID(orderId);
                setBooking(bookingg);
