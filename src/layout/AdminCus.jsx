...
import { Nguoidung, updatePer } from '../services/admin.services.js';

const AdminCus = function () {
    const [nguoidung, setNguoidung] = useState([]);
    const navigate = useNavigate();
    const [status, setStatus] = useState(nguoidung.mataikhoan);
    const { MT, MB, MDNB, MTNB ,User} = useContext(TravelMenu);
    const handleChange = async (e, mataikhoan) => {
        const selectedValue = e.target.value;
        if (selectedValue === 'user' || selectedValue === 'admin') {
            try {
                await updatePer({mataikhoan,per: selectedValue});
                setStatus(nguoidung.filter(nguoidung => nguoidung.mataikhoan !== mataikhoan));
                alert('PER status updated successfully!');
            } catch (error) {
                console.error('Error updating PER status:', error);
                alert('Failed to update PER status.');
            }
        }
    }
    useEffect(() => {
        ...
        <div id="content-wrapper" className={`${styles["d-flex"]} ${styles["flex-column"]}`}>
            <div id="content">
                ...
                <div className={styles['container-fluid']}>
                    <h1 className={`${styles.h3} ${styles['mb-2']} ${styles['text-gray-800']}`}>Danh Sách Người Dùng</h1>
                    <button className={`${styles['mb-4']}`}>
                        XUẤT RA EXCEL
                    </button>

                    <div className={`${styles.card} ${styles.shadow} ${styles['mb-4']}`}>
                        <div className={`${styles['card-header']} ${styles['py-3']}`}>
                            <h6 className={`${styles['m-0']} ${styles['font-weight-bold']} ${styles['text-primary']}`}>Danh sách Tour Người Dùng</h6>
                        </div>
                        <div className={`${styles['card-body']}`}>
                            <div className={`${styles['table-responsive']}`}>
                                <table className={`${styles.table} ${styles['table-bordered']}`} id='Travel' width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        {nguoidung.map(tt => (
                                            <tr key={tt.mataikhoan}>
                                                <td>{tt.mataikhoan}</td>
                                                <td><img src={tt.anhdaidien} alt="" /></td>
                                                <td>
                                                    <div className={styles.item}>
                                                        <p style={{ whiteSpace: 'nowrap' }}>Mã Người Dùng :</p>
                                                        <span style={{ whiteSpace: 'nowrap' }}>{tt.manguoidung}</span>
                                                    </div>
                                                    <div className={styles.item}>
                                                        <p style={{ whiteSpace: 'nowrap' }}>Tài Khoản :</p>
                                                        <span style={{ whiteSpace: 'nowrap' }}>{tt.taikhoan}</span>
                                                    </div>
                                                    <div className={styles.item}>
                                                        <p>Mật Khẩu :</p>
                                                        <span>{tt.matkhau}</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className={styles.item}>
                                                    ...
                                                    <div className={styles.item}>
                                                        <p style={{ whiteSpace: 'nowrap' }}>Ngày Sinh :</p>
                                                        <span style={{ whiteSpace: 'nowrap' }}>{tt.ngaysinh}</span>
                                                    </div>
                                                    <div className={styles.item}>
                                                        <p style={{ whiteSpace: 'nowrap' }}>Số Điện Thoại</p>
                                                        <span style={{ whiteSpace: 'nowrap' }}>{tt.sodienthoai}</span>
                                                    </div>
                                                    <div className={styles.item}>
                                                        <p>Email</p>
                                                        <span>{tt.email}</span>
                                                ...
                                                <td>
                                                    <div className={styles.item}>
                                                        <select name="" style={{ whiteSpace: 'nowrap' }} id="" onChange={(e) => handleChange(e, tt.mataikhoan)} value={tt.per}>
                                                            <option value="user" style={{ whiteSpace: 'nowrap' }}>user</option>
                                                            <option value="admin" style={{ whiteSpace: 'nowrap' }}>admin</option>
                                                        </select>
                                                    </div>
                                                </td>
                                                <td>
